...
/**
 * {@link MethodInterceptor} which creates a web service client using the
 * credentials from Spring Security's thread local bound {@link Authentication}
 * object. This is just a hack until the chaotic spring configuration of the
 * wicket components is properly handled. Each page should declare its
 * dependencies and there is should be clear if it needs a system client or an
 * agent authenticated one.
 * <p>
 ...
 */
public final class SmartClientInterceptor implements MethodInterceptor,
	InitializingBean {

    /** logger. */
    private static final Logger LOG = LoggerFactory
	    .getLogger(SmartClientInterceptor.class);
    ...
    /**
     * Whether self authentication is currently forced for this thread even if
     * the session does not contain a self authentication marker.
     *
	...
	if (this.systemAuthTarget == null) {
	    throw new IllegalStateException(
		    "systemAuthTarget array is required");
	}
    }
	...
	final SmartClientInterceptor otherProxy;
	if (other instanceof SmartClientInterceptor) {
	    otherProxy = (SmartClientInterceptor) other;
	} else {
    ...
    @Override
    public int hashCode() {
	return SmartClientInterceptor.class.hashCode() * 13
		+ this.selfAuthTarget.hashCode()
	...
	if (forced
		|| (Session.exists()
			&& Session.get() instanceof MobiliserWebSession && ((MobiliserWebSession) Session
		...
		LOG.warn("Application requires self authentication but "
			+ "customer has no authentication in the "
			+ "security context. Method: " + invocation.getMethod()
			+ " Will be called with a system client.",
		...
		if (StringUtils.indexOf(e.getMessage(),
			"Access is denied [403]") == StringUtils.INDEX_NOT_FOUND
			&& StringUtils.indexOf(e.getMessage(),
				"Forbidden [403]") == StringUtils.INDEX_NOT_FOUND) {
		    throw e;
		}
    ...
    private Object invoke(final MethodInvocation invocation, final Object target)
	    throws Throwable {
    ...
    /**
     * @param selfAuthTarget
     *            the selfAuthTarget to set
     */
    ...
    /**
     * @param systemAuthTarget
     *            the systemAuthTarget to set
     */
    ...
    private void warnAuthentication(final MethodInvocation invocation,
	    final Authentication authentication, final Exception e) {

	final Method method = invocation.getMethod();

	final RolesAllowed annotation = method
		.getAnnotation(RolesAllowed.class);

	if (annotation == null) {
	    LOG.trace(
		    "WARNING!!!!! Agent ({}) does not possess required "
			    + "privileges to invoke method {}"
			    + ". You should consider refactoring this "
			    + "wicket page to properly configure the needed "
			    + "clients. Falling back to system auth client "
			    + "[Exception: {}]",
		    new Object[] { authentication.getName(),
			    invocation.getMethod(), e.toString() });
	} else {
	    LOG.trace(
		    "WARNING!!!!! Agent ({}) does not possess required "
			    + "privileges ({}) to invoke method {}"
			    ...
			    + "[Exception: {}]",
		    new Object[] { authentication.getName(),
			    StringUtils.join(annotation.value()),
			    invocation.getMethod(), e.toString() });
	}
    }
}
