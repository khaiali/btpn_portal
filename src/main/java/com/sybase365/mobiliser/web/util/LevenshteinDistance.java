...
public class LevenshteinDistance {
    ...
    public static float percentageLevenshteinDistance(String expected,
	    String input) {
	expected = expected.toLowerCase().replaceAll(" ", "");
	input = input.toLowerCase().replaceAll(" ", "");
	float result = 1 - ((float) computeLevenshteinDistance(expected, input) / (float) expected
		.length());

	return result;
    }

    public static boolean checkSimilarityOfStrings(String expected,
