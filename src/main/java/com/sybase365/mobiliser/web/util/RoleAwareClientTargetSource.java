...
/**
 * {@link TargetSource} which creates a web service client using the credentials
 * from Spring Security's thread local bound {@link Authentication} object.
 * <p>
 ...
 */
public final class RoleAwareClientTargetSource implements TargetSource,
	InitializingBean, BeanClassLoaderAware {

    /** logger. */
    private static final Logger LOG = LoggerFactory
	    .getLogger(RoleAwareClientTargetSource.class);
	...
	if (this.clientInterfaces == null) {
	    throw new IllegalStateException(
		    "clientInterfaces array is required");
	}
	...
	final boolean forced = SmartClientInterceptor
		.isSelfAuthenticationForced();

	if (!forced && !Session.exists()) {
	    throw new IllegalStateException(
		    "Application misconfigured. This client requires a logged in user.");
	}
	...
	if (forced
	    ...
	    final Customer token = (Customer) authentication.getPrincipal();

	    return this.clientFactory.createClient(this.clientInterfaces,
		    this.configuration.getMobiliserEndpointUrl()
