...
public class SendMoneyFriendPage extends BaseSelfCarePage {

    private static final long serialVersionUID = 1L;

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory
	    .getLogger(SendMoneyFriendPage.class);
    ...
    public SendMoneyFriendPage() {
	super();
    }

    public SendMoneyFriendPage(final String recipient) {
	super();
	...
	Form<?> form = new Form("sendMoneyForm",
		new CompoundPropertyModel<SendMoneyFriendPage>(this));
	...
	if (getTxnText().length() > Integer
		.valueOf(Constants.MAX_LENGTH_SEND_MONEY_TXN_TEXT)) {
	...
	try {
		...
		if (handleTransaction(tab))
		    setResponsePage(new SendMoneyFriendConfirmPage(tab, false));
	    } else {
		if (startVoucher(tab))
		    setResponsePage(new SendMoneyFriendConfirmPage(tab, true));
	    }
	} catch (Exception e) {
	...
	tab.setAuthenticationMethodPayee(Integer.valueOf(response
		.getAuthenticationMethods().getAuthMethodPayee().getId()));
	tab.setAuthenticationMethodPayer(Integer.valueOf(response
		.getAuthenticationMethods().getAuthMethodPayer().getId()));
