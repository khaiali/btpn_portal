...
public class SendMoneyConfirmPage extends BaseTransactionsPage {

    private static final long serialVersionUID = 1L;

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory
	    .getLogger(SendMoneyConfirmPage.class);
    ...
    public SendMoneyConfirmPage(final TransactionBean tab,
	...
	Form<?> form = new Form("sendMoneyConfirm",
		new CompoundPropertyModel<SendMoneyConfirmPage>(this));
	...
	LOG.debug("SendMoneyConfirmPage.handleBack()");
	setResponsePage(SendMoneyPage.class);
    }

    private void sendMoneyConfirm() {
	LOG.debug("SendMoneyConfirmPage.sendMoneyConfirm()");
	try {
	    if (!isUnknownPayee) {
    ...
    private boolean confirmVoucher(TransactionBean transaction)
	    throws Exception {
	LOG.debug("# SendMoneyPage.confirmVoucher()");
	...
	if (response.getStatus().getCode() == Constants.PENDING_APPROVAL_ERROR) {
	    LOG.warn("#  Your voucher request has been processed successfully, but is pending approval");
	    transaction.setStatusCode(Constants.TXN_STATUS_PENDING_APPROVAL);
	    return false;
	}
