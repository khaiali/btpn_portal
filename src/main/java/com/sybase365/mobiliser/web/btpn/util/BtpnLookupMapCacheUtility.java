...
import com.sybase365.mobiliser.web.btpn.application.pages.BtpnBasePage;
import com.sybase365.mobiliser.web.util.PortalUtils;

public class BtpnLookupMapCacheUtility implements ILookupMapUtility, InitializingBean {
	private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(BtpnLookupMapCacheUtility.class);
	...
	public Map<String, String> getLookupNamesMap(String lookupMapName, Component component) {
		...
		try {
			if (LOG.isTraceEnabled()) {
				LOG.trace("Loading lookup names from  btpn service");
			}
			...
			if (response.getStatus().getCode() != 0) {
				...
				props = component.getApplication().getResourceSettings().getPropertiesFactory()
					.load(BtpnBasePage.class, BtpnBasePage.class.getName().replace('.', '/') + '.');
