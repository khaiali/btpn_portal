...
import com.btpnwow.core.limitex.services.contract.v1_0.UpdateLimitExRequest;
import com.btpnwow.core.limitex.services.contract.v1_0.UpdateLimitExResponse;
...
import com.sybase365.mobiliser.web.btpn.bank.pages.portal.limit.ElimitEditSuccessPage;
 ...
 */
public class ElimitEditConfirmPanel extends Panel {

	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory.getLogger(ElimitEditConfirmPanel.class);
	...
	public ElimitEditConfirmPanel(String id, BtpnBaseBankPortalSelfCarePage basePage, ElimitBean cashOutBean) {
		super(id);
		this.basePage = basePage;
		this.mobBasePage= basePage;
		this.limitBean = cashOutBean;
		constructPanel();
	}

	protected void constructPanel() {

		final Form<BankPortalCashinDetailsPanel> form = new Form<BankPortalCashinDetailsPanel>("limitEditConfirmForm",
	 ...
	 */
	private void handleUpdateLimitBean() {
		try {
			UpdateLimitExRequest request = this.mobBasePage.getNewMobiliserRequest(UpdateLimitExRequest.class);
			LimitExType limit = ConverterUtils.convertToLimitExType(limitBean);
			request.setData(limit);
			UpdateLimitExResponse response = this.mobBasePage.limitExClient.update(request);
			if (this.mobBasePage.evaluateBankPortalMobiliserResponse(response)) {
				this.mobBasePage.getWebSession().info(getLocalizer().getString("update.success.waiting.approval", this));
				setResponsePage(new ElimitEditSuccessPage(limitBean));
			} else {
