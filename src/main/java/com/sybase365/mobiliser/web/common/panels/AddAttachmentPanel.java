...
public class AddAttachmentPanel extends Panel {
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory
	    .getLogger(AddAttachmentPanel.class);
    ...
    public AddAttachmentPanel(String id, Long customerId,
	...
	LOG.info("Created new AddAttachmentsPanel");
	...
	final Form form = new Form("addAttachmentForm",
		new CompoundPropertyModel<AddAttachmentPanel>(this));

	form.setMultiPart(true);

	form.add(upload = new FileUploadField("uploadAttachmentFile"));
	form.addOrReplace(new TextField<String>("attachmentName")
		.add(Constants.mediumStringValidator)
		.add(Constants.mediumSimpleAttributeModifier)
		.add(new ErrorIndicator()));

	form.add(new LocalizableLookupDropDownChoice<Integer>("attachmentType",
		Integer.class, "attachmenttypes", this, false, true) {
	    @Override
	...
	form.add(new AjaxButton("addAttachment") {
		...
		} else if (!PortalUtils.exists(attachmentName)) {
		    attachmentName = upload.getFileUpload().getClientFileName();
		}
		if (requiredStatus) {
		    addAttachment();
		    attachmentType = null;
	...
	if (upload != null) {
		...
		if (PortalUtils.exists(attachmentType)) {
	...
	try {
	    ...
	    forceReload = true;
	    info(getLocalizer().getString("attachments.delete.success", this));

	} catch (Exception e) {
	    LOG.error("An error occurred for delete attachment", e);
	...
	final DataView<Attachment> dataView = new DataView<Attachment>(
		...
		item.add(new Label(WICKET_ID_type, getLocalizer().getString(
			"addAttchment.type." + entry.getAttachmentType(), this)));
	...
	parent.add(new MultiLineLabel(WICKET_ID_noItemsMsg, getLocalizer()
		.getString("addAattachments.table.noItemsMsg", this)
		+ "\n"
		+ getLocalizer().getString(
			"addAttachments.table.addAttachmentHelp", this)) {
