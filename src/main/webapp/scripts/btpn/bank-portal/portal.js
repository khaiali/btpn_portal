...
function getSelectedMenuName() {	
	var ulTop = document.getElementById("topMenuList");
	var liArrayTop = ulTop.getElementsByTagName("li");
	for ( var i = 0; i < liArrayTop.length; i++) {
		if (liArrayTop[i].getAttribute("class") == "selected") {
			var anchor = liArrayTop[i].getElementsByTagName("a");
...
/*
 * Key enter for Registration Panel
 * 
 */
function keyEnter(btn) {
	if (event.keyCode == 13) {
